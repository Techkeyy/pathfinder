...
This PR affects **5 downstream assets** across your stack.

### `orders`, renamed `customer_id` -> `cust_id`
_renamed `customer_id` -> `cust_id`: touches 5 downstream assets, including 4 in PRODUCTION (customer_value, daily_orders, Exec Revenue). Treat as breaking._
...
<sub>Pathfinder walked your DataHub lineage graph. Verdicts and rationale are deterministic; the drafted fix may be LLM-polished.</sub>
